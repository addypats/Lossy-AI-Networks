...
# --- Layer-shape-wise packet logging state ---
# We treat each unique (global_elems, dtype) as a "layer shape".

# Map: layer_key -> {"layer_id", "global_elems", "dtype", "ag_packets", "rs_packets", "written"}
_LAYER_PACKET_INFO = {}
_NEXT_LAYER_ID = 0
...
def _write_layer_row(entry: dict) -> None:
    """
    Append a single row for this "layer shape" to the CSV:
...
def _record_packets(fn_name: str, t: torch.Tensor, num_packets: int) -> None:
    """
    Associate this call (AG or RS) with a "layer shape" using (global_elems, dtype),
    and record packet counts. Once both AG and RS are known for that shape, log a row.
    """
    ...
    layer_key = (global_elems, t.dtype)

    if layer_key not in _LAYER_PACKET_INFO:
        _LAYER_PACKET_INFO[layer_key] = {
    ...
    entry = _LAYER_PACKET_INFO[layer_key]

    if fn_name == "all_gather_into_tensor" and entry["ag_packets"] is None:
        entry["ag_packets"] = num_packets
    elif fn_name == "reduce_scatter_tensor" and entry["rs_packets"] is None:
...
def install_lossy_collectives(
    ...
    log per-layer-shape packet counts.
                ...
                if fn_name == "all_gather_into_tensor" and enable_allgather:
                    ...
                    if len(args) >= 2 and isinstance(args[1], torch.Tensor):
                        ...
                        if _is_payload_tensor(t) and t.numel() >= min_numel:
                            ...
                            if num_packets > 0:
                                _record_packets(fn_name, t, num_packets)
