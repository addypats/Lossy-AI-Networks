...
def train_step(model: nn.Module,
               ...
               step: int):
    """
    Performs a single training step (forward, backward, gradient syncing via LossyNetwork, optimizer step).
    """
    model.train()
    if use_fp16:
        with autocast():
...
def evaluate(model: nn.Module, eval_loader: DataLoader, device: torch.device):
    """
    Standard evaluation loop: gather local correct / total, then all-reduce to compute global accuracy.
    """
    model.eval()
    local_correct = torch.tensor(0, device=device, dtype=torch.long)
    local_total = torch.tensor(0, device=device, dtype=torch.long)
...
def train_to_accuracy(args):
    ...
    # -------------------------------------------
    # 2) Read dataset config & create dataloaders
    # -------------------------------------------
    with open("src/config.yaml") as cf:
        dataset_config = yaml.safe_load(cf)[args.dataset]
    ...
    # -------------------------------------------
    # 3) Initialize Weights & Model
    # -------------------------------------------
    if local_rank == 0:
        wandb.init(
            project="lossy-llama3-finetune",
            config=vars(args),
    ...
    # 7) Training Loop
    # -------------------------------------------
    best_acc, no_improve, step = 0.0, 0, 0
    ...
    while step < args.max_steps:
        ...
        for batch in tqdm(train_loader,
                          desc=f"Rank {local_rank} Step {step}",
            ...
            if step % args.eval_steps == 0:
                ...
                # Decide if we should stop (on rank 0) then broadcast
                stop_signal = torch.tensor(0, dtype=torch.uint8, device=device)
                if local_rank == 0:
    ...
    # Save final when max_steps reached
    if local_rank == 0:
        torch.save(model.state_dict(), os.path.join(args.output_dir, "model_final.pt"))
        with open(os.path.join(args.output_dir, "metrics.json"), "w") as mf:
            json.dump(metrics, mf, indent=2)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="TP Finetune Llama3.2-1B with LossyNetwork")
    parser.add_argument("--local_rank", type=int, default=0)
    parser.add_argument("--max_length", type=int, default=256)
    parser.add_argument("--tensor_parallel_size", type=int, default=4)
    parser.add_argument("--model_name", type=str, default="meta-llama/Llama-3.2-1B")
