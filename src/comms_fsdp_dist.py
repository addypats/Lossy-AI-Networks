...
# fsdp dist training receive function

    def receive(self, data: torch.Tensor, packets_mask: torch.Tensor) -> torch.Tensor:
        """
        Apply packet loss in-place to `data` based on `packets_mask`.

        - `packets_mask` is length = num_packets (True = kept, False = dropped)
        - We assume contiguous packing of elements into packets.
        - We expand to a per-element mask without building giant index tensors.
        """
        # Fast path: no loss
        if packets_mask is None or packets_mask.numel() == 0 or packets_mask.all():
            return data

        flat = data.view(-1)
        elem_size = data.element_size()

        # How many elements per packet (upper bound)
        number_per_packet = MAX_PAYLOAD_BYTES // elem_size + 1

        # Expand packet mask to per-element mask
        # (length will be >= flat.numel(), then we crop)
        # NOTE: packets_mask is usually on CPU; build on CPU, then move once.
        per_elem_mask = packets_mask.repeat_interleave(number_per_packet)[: flat.numel()]
        per_elem_mask = per_elem_mask.to(flat.device)

        # Zero out "dropped" elements in-place
        flat[~per_elem_mask] = 0.0
